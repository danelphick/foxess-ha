...
class FoxESSSchedulerCard extends HTMLElement {
  ...
  _render() {
    ...
    const toSeg = s => {
      ...
      return `<div class="seg${s.isActive ? ' seg-active' : ''}" style="left:${l}%;width:${w}%;background:${s.cfg.color}" title="${s.cfg.label}: ${s.start}–${s.end}"></div>`;
    };
    ...
    const makeRow = (s, label, showTime) => {
      const color = s.cfg?.color ?? '#9e9e9e';
      ...
      return `<tr class="${s.isActive ? 'row-active' : ''} ${!s.enabled ? 'row-off' : ''}">
    ...
    const remainingRow = remainingSlot
      ? `<tr><td colspan="7" class="remaining-sep"></td></tr>${makeRow(remainingSlot, 'Remaining Time Slots', false)}`
      : '';
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .seg { position:absolute; height:100%; border-radius:4px; opacity:.75; cursor:default; }
        ...
        .num { color:var(--secondary-text-color); width:20px; text-align:center; }
      ...
      <ha-card>
        ...
        <table>
          <thead><tr><th>#</th><th>Mode</th><th>Start</th><th>End</th><th>Min SoC</th><th>FD SoC</th><th>FD Power</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </ha-card>`;
  }
